import { AppText as Text } from '@/components/app-text';
import { scale, verticalScale } from '@/constants/scaling';
import { Fonts } from '@/constants/theme';
import { Ionicons } from '@expo/vector-icons';
import React from 'react';
import { Image, StyleSheet, View } from 'react-native';

const CheckItem = ({ text }: { text: string }) => (
  <View style={styles.bulletItem}>
    <View style={styles.checkIconContainer}>
      <Ionicons name="checkmark" size={12} color="#1C1B1F" />
    </View>
    <Text style={styles.bulletText}>{text}</Text>
  </View>
);

const ProtocolCard = ({
  title,
  subtitle,
  items,
  icon,
  bgColor
}: {
  title: string,
  subtitle: string,
  items: string[],
  icon: any,
  bgColor: string
}) => (
  <View style={[styles.card, { backgroundColor: bgColor }]}>
    <View style={styles.cardContent}>
      {/* Icon */}
      <View style={styles.iconContainer}>
        <Image source={icon} style={styles.icon} resizeMode="contain" />
      </View>

      {/* Content */}
      <View style={styles.textContent}>
        <View style={styles.cardHeader}>
          <Text style={styles.cardTitle}>{title}</Text>
          <Text style={styles.cardSubtitle}>{subtitle}</Text>
        </View>

        {/* Bullet List */}
        <View style={styles.bulletList}>
          {items.map((item, index) => (
            <CheckItem key={index} text={item} />
          ))}
        </View>
      </View>
    </View>
  </View>
);

export function ProtocolSection() {
  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>Your Complete Protocol</Text>

      <ProtocolCard
        title="Primary Treatment Formula"
        subtitle="Targeted active ingredients designed to treat your core skin concerns."
        items={[
          "Reduces acne-causing bacteria",
          "Unclogs pores",
          "Improves texture & tone"
        ]}
        icon={require('@/assets/images/protocol-primary.png')}
        bgColor="#EEE2D6"
      />

      <ProtocolCard
        title="Supplement Support"
        subtitle="Helps your skin heal and strengthen from within."
        items={[
          "Balances inflammation",
          "Boosts recovery",
          "Supports long-term results"
        ]}
        icon={require('@/assets/images/protocol-supplement.png')}
        bgColor="#EDEAE4"
      />

      <ProtocolCard
        title="Home Device"
        subtitle="Enhances absorption and maximizes treatment effectiveness."
        items={[
          "Improves product penetration",
          "Smooths texture",
          "Speeds up visible results"
        ]}
        icon={require('@/assets/images/protocol-device.png')}
        bgColor="#F2EEFF"
      />
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    width: '100%',
    // maxWidth removed
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    padding: 12, // padding: 12px
    gap: 8, // gap: 8px
  },
  sectionTitle: {
    fontFamily: Fonts.medium, // weight: 500
    fontSize: scale(20),
    lineHeight: scale(24),
    color: '#20201E',
    marginBottom: 8, // Little extra spacing from title to first card visually, though gap handles it
    marginLeft: 4, // Align with card content slightly
  },
  card: {
    width: '100%',
    borderRadius: 8,
    padding: 16, // using padding instead of absolute positioning for content
    minHeight: verticalScale(181),
  },
  cardContent: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    gap: 16,
  },
  iconContainer: {
    width: 40,
    height: 40,
    backgroundColor: '#FFFFFF',
    borderRadius: 8,
    alignItems: 'center',
    justifyContent: 'center',
  },
  icon: {
    width: 28,
    height: 28,
  },
  textContent: {
    flex: 1,
    gap: 16,
  },
  cardHeader: {
    gap: 8,
  },
  cardTitle: {
    fontFamily: Fonts.medium, // weight: 500
    fontSize: scale(18),
    lineHeight: scale(22),
    color: '#20201E',
  },
  cardSubtitle: {
    fontFamily: Fonts.regular, // weight: 400
    fontSize: scale(13),
    lineHeight: scale(18), // 140% of 13 is ~18.2
    color: '#20201E',
    opacity: 0.8,
  },
  bulletList: {
    gap: 8,
  },
  bulletItem: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  },
  checkIconContainer: {
    width: 16,
    height: 16,
    alignItems: 'center',
    justifyContent: 'center',
  },
  bulletText: {
    fontFamily: Fonts.regular, // weight: 400
    fontSize: scale(13),
    lineHeight: scale(17), // 130% approx
    color: '#20201E',
  },
});
